...
public class WebdavStatus {
    ...
    static {
        ...
        temp.put(SC_OK, "OK");
        temp.put(SC_CREATED, "Created");
        temp.put(SC_ACCEPTED, "Accepted");
        temp.put(SC_NO_CONTENT, "No Content");
        temp.put(SC_MOVED_PERMANENTLY, "Moved Permanently");
        temp.put(SC_MOVED_TEMPORARILY, "Moved Temporarily");
        temp.put(SC_NOT_MODIFIED, "Not Modified");
        temp.put(SC_BAD_REQUEST, "Bad Request");
        temp.put(SC_UNAUTHORIZED, "Unauthorized");
        temp.put(SC_FORBIDDEN, "Forbidden");
        temp.put(SC_NOT_FOUND, "Not Found");
        temp.put(SC_INTERNAL_SERVER_ERROR, "Internal Server Error");
        temp.put(SC_NOT_IMPLEMENTED, "Not Implemented");
        temp.put(SC_BAD_GATEWAY, "Bad Gateway");
        temp.put(SC_SERVICE_UNAVAILABLE, "Service Unavailable");
        temp.put(SC_CONTINUE, "Continue");
        temp.put(SC_METHOD_NOT_ALLOWED, "Method Not Allowed");
        temp.put(SC_CONFLICT, "Conflict");
        temp.put(SC_PRECONDITION_FAILED, "Precondition Failed");
        temp.put(SC_REQUEST_TOO_LONG, "Request Too Long");
        temp.put(SC_UNSUPPORTED_MEDIA_TYPE, "Unsupported Media Type");
        // WebDav Status Codes
        temp.put(SC_MULTI_STATUS, "Multi-Status");
        temp.put(SC_UNPROCESSABLE_ENTITY, "Unprocessable Entity");
        temp.put(SC_INSUFFICIENT_SPACE_ON_RESOURCE,
                         "Insufficient Space On Resource");
        temp.put(SC_METHOD_FAILURE, "Method Failure");
        temp.put(SC_LOCKED, "Locked");
        
        mapStatusCodes = Collections.unmodifiableMap(temp);
    }
    ...
    public static String getStatusText(int nHttpStatusCode) {
        String retval = mapStatusCodes.get(nHttpStatusCode);
        if (retval==null)
            return "";
        return retval;
    }
